...
def main():
	...
	for scenario in sys.argv[2:]:
		if scenario == 'base':
			print('cannot unlock base scenario')
			continue
		if not scenario_exists(bucket, scenario):
			print(f'scenario {scenario} doesnt exist')
			continue
		if scenario not in lock_list:
			print(f'scenario {scenario} already unlocked')
			continue

		lock_list = [el for el in lock_list if el != scenario]
		print(f'unlocking: {scenario}')
