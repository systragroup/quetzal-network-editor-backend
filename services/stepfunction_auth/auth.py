...
def get_policy_document(policy_arn):
	...
	policy_version = iam_client.get_policy_version(PolicyArn=policy_arn, VersionId=policy['Policy']['DefaultVersionId'])
...
def handler(event, context):
	...
	for policy in policies:
		...
		print(resource)
		if (policy[1]['Effect'] == 'Allow') and (model in str(resource)):
			print('Allowed')
			break
		if resource == 'arn:aws:s3:::quetzal-*/*':
			print('Admin allowed')
			break
