...
resource "null_resource" "image" {
  ...
  provisioner "local-exec" {
    ...
    when    = create
    command = var.os == "windows" ? "cmd /c ../modules/ecr/push_dummy.bat ${aws_ecr_repository.repository.repository_url}" : " bash ../modules/ecr/push_dummy.sh ${aws_ecr_repository.repository.repository_url}  ${aws_ecr_repository.repository.name}"
  }
